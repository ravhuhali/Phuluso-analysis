...
def plot_weighted_age_distribution(df, age_column, weight_column, province_column):
    ...
    for axes, province in zip(axes, df[province_column].unique()):
        ...
        # Calculate the 95th and 25th percentiles using the inverted CDF method
...
def plot_age_distribution(df, age_column, weight_column):
    ...
    print(
        f"The age that owns the most houses (weighted by {weight_column}) is {highest_age[age_column]}"
    )


# Example usage:
# plot_age_distribution(df, 'head_age', 'house_wgt')
...
def metro_df(df, province_col, Q62cell, GeoType):
    ...
    # df["metro_name"] = df[Q62cell].map(metro_map)
